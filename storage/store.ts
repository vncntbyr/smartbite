...
export const addToHistory = async (
	value: HistoryData | undefined,
): Promise<void> => {
	try {
		if (!value) return;
		const currentHistory = (await readHistory()) || [];
		const currentDate = new Date().setHours(0, 0, 0, 0); // Set time to midnight for comparison
		...
		if (!todayHistory) {
			...
			!todayHistory.data.some((data) => data.barcode === value.barcode)
		) {
