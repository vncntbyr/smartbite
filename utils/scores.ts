...
export const getScores = (data: any): Scores => {
	const { grade, negative_points, positive_points, score } =
...
const getPlantScore = (ingredients: any): PlantScore => {
	const veganIngredients = ingredients.some(
		(ingredient: any) => ingredient.vegan === "no",
	)
		? 2
		: 0;
...
const getNovaScore = (value: NovaScore["value"]): NovaScore => {
	const color = getColorForNumericScore(Number.parseInt(value));
	return {
		value,
		color,
	};
